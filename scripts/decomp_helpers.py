...
def cmd_trim_context(c_file, *func_names):
    ...
    """
    try:
        lines = Path(c_file).read_text(encoding="utf-8", errors="replace").splitlines()
    except FileNotFoundError:
...
def cmd_extract_asm(asm_file, func_name):
    ...
    try:
        text = Path(asm_file).read_text(encoding="utf-8", errors="replace")
    except FileNotFoundError:
...
def cmd_extract_struct(types_header, struct_name):
    ...
    """
    try:
        lines = Path(types_header).read_text(encoding="utf-8", errors="replace").splitlines()
    except FileNotFoundError:
